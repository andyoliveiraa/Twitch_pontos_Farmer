...
# Load all optional notifications settings from environment variables
...
matrix_instance = None
if matrix_username and matrix_homeserver:
...
pushover_userkey = os.getenv("PUSHOVER_USER_KEY")
pushover_token = os.getenv("PUSHOVER_TOKEN")
pushover_instance = None
if pushover_userkey and pushover_token:
    pushover_instance = Pushover(
...
gotify_endpoint = os.getenv("GOTIFY_ENDPOINT")
gotify_instance = None
if gotify_endpoint:
    gotify_instance = Gotify(
...
twitch_miner = TwitchChannelPointsMiner(
    username=os.getenv("TWITCH_USERNAME", "your_twitch_username"),
    password=os.getenv("TWITCH_PASSWORD") or None,           # If no password will be provided, the script will ask interactively
    ...
    disable_ssl_cert_verification=True,        # Set to True at your own risk and only to fix SSL: CERTIFICATE_VERIFY_FAILED error
    ...
    logger_settings=LoggerSettings(
        ...
        discord=discord_instance,
        webhook=webhook_instance,
        matrix=matrix_instance,
        pushover=pushover_instance,
        gotify=gotify_instance
    ),
...
twitch_miner.analytics(host="0.0.0.0", port=port, refresh=5, days_ago=7)   # Start the Analytics web-server
